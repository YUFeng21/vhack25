...
import {
    ...
    GenTableChatCompletionChunksSchema,
...
export class GenTable extends Base {
    ...
    public async addActionColumns(params: AddColumnRequest): Promise<TableMetaResponse> {
    ...
    public async addRowStream({ table_id, table_type, data, reindex = true }: AddRowRequest) {
        const apiURL = `/api/v1/gen_tables/${table_type}/rows/add`;
    ...
    public async addRow({ table_id, table_type, data, reindex = true }: AddRowRequest): Promise<GenTableChatCompletionChunks> {
        const url = `/api/v1/gen_tables/${table_type}/rows/add`;
        const response = await this.httpClient.post(
            url,
            {
                table_id: table_id,
        ...
        return new Promise((resolve, reject) => {
            if (response.status == 200) {
                const parsedData = GenTableChatCompletionChunksSchema.parse(response.data);
                resolve(parsedData);
            } else {
                console.error("Received Error Status: ", response.status);
            }
    ...
    public async regenRow(params: RegenRowRequest): Promise<GenTableChatCompletionChunks> {
        const apiURL = `/api/v1/gen_tables/${params.table_type}/rows/regen`;
        const response = await this.httpClient.post(
            apiURL,
            {
